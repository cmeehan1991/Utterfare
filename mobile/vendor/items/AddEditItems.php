...
function getItems() {
    ...
    $limit = null;
    $customer_id = filter_input(INPUT_POST, 'COMPANY_ID');//$_SESSION["COMPANY_ID"];
    $data_table = filter_input(INPUT_POST, 'DATA_TABLE'); // $_SESSION['DATA_TABLE'] . "_items"; // This is only the prefix ###ST 
        ...
        /*while ($results = $stmt->fetch()):
            ...
            <tr class="<?php echo $results["ID"]; ?>" onclick="return editItem(<?php echo $results["ID"] ?>)" >
		        ...
		        $item_image = $results['ITEM_IMAGE'];
		       	if($item_image != null && strpos($item_image, 'https') > -1 && strpos(get_headers($item_image)[0], '200 OK') > -1): 
		       	?>
