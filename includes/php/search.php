...
class Item_Search{
	...
	function __construct(){
		$this->init();
		//echo $this->getResults($this->location, $this->distance, $this->terms, $this->page);
	}
	...
	private function init(){
		...
		$this->search_analytics = new SearchAnalytics();
		$this->search_analytics->save_search_terms($this->terms);
		$this->search_id = $this->search_analytics->save_search_general_information($this->location, $this->distance, $this->terms, $this->type, $this->platform);	
		*/
	...
	private function doSearch(){
        ...
        echo $this->search($distance, $lat, $lng, $limit, $page, $distance, $offset, $terms);
	}
	...
	*/
	private function getRecommendations(){
		$search_location = filter_input(INPUT_POST, 'location');				
		$distance = 10;
	...
	private function search($distance = 10, $latitude = null, $longitude = null, $ppp = null, $page = 1, $offset = 0, $terms = null){
		...
		$sql = "SELECT DISTINCT item_id, item_name, item_short_description, primary_image, vendors.vendor_name
		FROM menu_items 
	...
	private function getDataTables($location, $distance) {
	...
	*/	
	...
	private function getResults($location, $distance, $terms, $page) {
	    include 'DbConnection.php';
	    // Get the datatables to be used for the search. 
	    $data_tables = $this->getDataTables($location, $distance);
	    ...
	    if ($num_rows > 0) {
	        ...
	        usort($sorted_results, array($this, 'rankItems'));
	        ?>     
	        ...
	        <div class="container-fluid">
	            <?php
		           	...
		           	$counter = 0;
					foreach ($sorted_results as &$result) {
						$this->search_analytics->save_search_result_data($this->search_id, $page, $result['DATA_TABLE'], $result['ITEM_ID'], $result['COMPANY_ID']);
						if($counter < 10){
	...
	private static function rankItems($a, $b) {
